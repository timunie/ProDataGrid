...
{
    /// <summary>
    /// Search adapter that relies on column value accessors to avoid reflection.
    /// </summary>
    public sealed class AccessorSearchAdapter : DataGridSearchAdapter
    {
        private readonly Func<IEnumerable<DataGridColumn>> _columnProvider;

        public AccessorSearchAdapter(
            ISearchModel model,
        ...
        private List<SearchColumnInfo> BuildColumnInfos()
        {
            ...
            foreach (var column in columns)
            {
                ...
                var info = CreateColumnInfo(column, index);
                if (info != null)
                {
                    list.Add(info);
                }
        ...
        private SearchColumnInfo CreateColumnInfo(DataGridColumn column, int fallbackIndex)
        {
            ...
            {
                propertyPath = GetSortPropertyName(column);
            }
            ...
            var columnIndex = column.DisplayIndex >= 0 ? column.DisplayIndex : fallbackIndex;
        ...
        private static bool IsSameColumnId(object id, SearchColumnInfo column)
        {
            ...
            if (id is DataGridColumnDefinition definition)
            {
                ...
                if (!string.IsNullOrEmpty(definitionPath) &&
                    string.Equals(definitionPath, column.PropertyPath, StringComparison.Ordinal))
                {
                    return true;
                }
            }
